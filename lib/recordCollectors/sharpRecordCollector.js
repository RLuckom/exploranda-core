...
function transform({sharp}, sourceSchema, paramSet) {
  ...
  if (sourceSchema.returnsPromise) {
    return function(input, callback) {
      ...
      args.push(paramSet)
      apiObject[sourceSchema.apiMethod].apply(apiObject, args).toBuffer().then((buf) => callback(null, buf), callback)
    }
  } else { 
...
module.exports = {
  transform: buildSDKCollector({getApi: transform, dependencyMap: {sharp: 'sharp'}})
}
