 ...
 */
function getApi({google}, sourceSchema, paramSet) {
  let auth = _.get(paramSet, 'auth');
  if (_.get(paramSet.apiConfig, 'keyFilename') || _.get(paramSet.apiConfig, 'keyJson')) {
    auth = paramSet.apiConfig.keyFilename ? google.auth.fromJSON(require(paramSet.apiConfig.keyFilename)) : google.auth.fromJSON(paramSet.apiConfig.keyJson);
...
module.exports = {
  lookUpRecords: buildSDKCollector({ getApi, dependencyMap: { google: 'googleapis'}})
};
