...
function genericRequestApi(sourceSchema) {
  return function(params, callback) {
    ...
    function detectErrors(e, r, b) {
      const statusCode = _.get(r, 'statusCode');
      const statusCodeFailure = (200 > statusCode) || (300 <= statusCode);
      if (statusCodeFailure) {
        if (process.env.EXPLORANDA_DEBUG) {
          return `Status code ${statusCode} received for request ${JSON.stringify(mergedRequestParams)} with responseBody ${JSON.stringify(b)} and error ${e}`
        } else {
          return `Status code ${statusCode} received for request. To see more detail re-run with EXPLORANDA_DEBUG=true in the environment.`
        }
      }
    }
    ...
    return request(
      mergedRequestParams,
      (e, r, b) => {
        callback(e || (sourceSchema.detectErrors || detectErrors)(e, r, b), {
          body: b,
