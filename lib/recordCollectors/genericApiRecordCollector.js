...
function genericRequestApi(sourceSchema) {
  function api(params, callback) {
    ...
    const requestUrl =  addQueryStringToUrl(urlBuilderFunction(pathParams), queryObject)
    ...
    return needle(method, requestUrl, _.keys(requestData).length > 0 ? requestData : void(0),
      mergedRequestParams,
      (e, r, b) => {
        callback(e, 
                 {
