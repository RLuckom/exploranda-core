...
function buildPopulateNamespaceObject(getApi, dependencies) {
  return function populateNamespaceObject(sourceSchema, paramsArray, behaviors, callback) {
    let apiConfig = _.reduce(paramsArray, (acc, v) => {
      if (!_.isEqual(v.apiConfig, acc)) {
        throw new Error(`For accessSchemas with namespaceDetails.isTarget set, every paramSet in a paramArray must have an identical apiConfig.`)
      }
      return v.apiConfig
    }, _.get(paramsArray, '[0].apiConfig'))
    const api = getApi(sourceSchema, {apiConfig, paramsArray}, behaviors);
    async.series(_.map(paramsArray, (paramSet) => {
      return function(seriesCallback) {
        api(paramSet, seriesCallback)
      }
    }), (e, r) => callback(e, _.get(r, '[0]')))
  }
}
...
function execParamDrivenRequest(sourceSchema, paramsArray, behaviors, callback) {
  if (sourceSchema.isSync) {
    async.series(_.map(paramsArray, (params) => {
      return function(seriesCallback) {
        const argOrder = _.get(sourceSchema, 'argumentOrder')
        if (argOrder) {
          const args = _.map(argOrder, (a) => params[a])
          setTimeout(() => {
            try {
              seriesCallback(null, params.apiConfig(...args))
            } catch (e) {
              seriesCallback(e)
            }
          }, 0)
...
function buildExecRecordsRequest(fetchAndMergeAllResults, populateNamespaceObject) {
  return function execRecordRequest(sourceSchema, params, behaviors, callback) {
    ...
    if (_.get(sourceSchema, 'namespaceDetails.paramDriven')) {
      ...
      return
    } else if (_.get(sourceSchema, 'namespaceDetails.isTarget')) {
      return populateNamespaceObject(sourceSchema, paramsArray, behaviors, callback)
    } else {
...
function buildSDKCollector({getApi, dependencyMap}) {
  ...
  if (missing.length === 0) { 
    ...
    const populateNamespaceObject = buildPopulateNamespaceObject(_.partial(getApi, suppliedDependencies));
    const execRecordRequest = buildExecRecordsRequest(fetchAndMergeAllResults, populateNamespaceObject);
