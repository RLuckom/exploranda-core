...
function buildFetchAndMerge(getApi) {
  return function fetchAndMergeAllResults(sourceSchema, paramSet, behaviors, callback) {
    ...
    if (!api) {
      throw new Error(`Could not find API for source schema ${sourceSchema.name} and params supplied ${JSON.stringify(paramSet)}, ${JSON.stringify(behaviors)} ${callback} ${sourceSchema}`)
      return callback(
        new Error(`Could not find API for source schema ${sourceSchema.name} and params supplied`)
      );
    }
