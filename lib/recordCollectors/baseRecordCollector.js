    ...
    function fetchAndMergeAll(collector, err, res) {
      ...
      if (incompleteIndicatorFunction(res)) {
        ...
        if (_.isArray(currentParams)) {
          ...
          return retryRequest(currentParams, api, fetchAndMergeAll, behaviors, sourceSchema, collector);
        }
      } else {
        return callback(err, collector);
      }
    }
    return retryRequest(currentParams, api, fetchAndMergeAll, behaviors, sourceSchema, null);
  };
}
...
function retryRequest(currentParams, api, fetchAndMergeAll, behaviors, sourceSchema, collector) {
  return async.retry(
    ...
    (cb) => {
      ...
      return api(currentParams, errorDetection);
    },
